...
class BiPOTrainer(Trainer):
    ...
    @tokenizer.setter
    def tokenizer(self, value: Optional[PreTrainedTokenizerBase]) -> None:
        self._tokenizer = value
        # Mirror into processing_class for newer transformers versions while remaining backward compatible.
        try:
    ...
    def __init__(
        ...
        elif self.is_peft_model or args.precompute_ref_log_probs or getattr(args, "single_model_ref", False):
        ...
        if self.ref_model is None:
            if not (self.is_peft_model or self.precompute_ref_log_probs or getattr(self.args, "single_model_ref", False)):
                raise ValueError(
    ...
    def null_ref_context(self):
        ...
        Besides PEFT adapter manipulation, also temporarily disables the BiPO steering
        vector by setting the target layer's multiplier to 0.0 if available. This ensures
        reference log-probabilities are computed on the unsteered model using a single
        model instance.
        """
        # Handle PEFT adapter switching if applicable
        peft_ctx = (
        ...
        # Best effort: temporarily zero the steering multiplier on the target layer
        layer_module = None
        old_multiplier = None
        try:
            layer_module = self.model.model.layers[self.layer]
            if hasattr(layer_module, "set_multiplier"):
                old_multiplier = getattr(layer_module, "multiplier", None)
                layer_module.set_multiplier(0.0)
        except Exception:
            # If the model doesn't expose layers/multiplier, proceed without modification
            pass
        ...
        with peft_ctx:
            ...
            try:
                ...
                # Restore previous multiplier if it was modified
                try:
                    if layer_module is not None and hasattr(layer_module, "set_multiplier") and old_multiplier is not None:
                        layer_module.set_multiplier(old_multiplier)
                except Exception:
                    pass
    ...
    def concatenated_forward(
        ...
        """
        # Ensure inputs are placed on the same device as the passed `model` to avoid
        # CPU/GPU mismatches during precompute or evaluation contexts.
        try:
    ...
    def sample_batch_outputs(self, model, batch: Dict[str, torch.LongTensor]) -> Tuple[str, str]:
    ...
    def evaluation_loop(self, dataloader, description, prediction_loss_only=None, 
        ...
        # Print only on main process to reduce noise
        ...
        # Moved vector saving to a training-step callback to avoid saving during evaluation.

        # For the sampling part, avoid deepcopy by creating new instances
        if self.generate_during_eval and self.is_world_process_zero() and is_wandb_available():
            num_samples = len(dataloader.dataset)
            k = min(self.args.eval_batch_size, num_samples)
            random_indices = random.sample(range(num_samples), k=k) if k > 0 else []
            
            # Create a new dataset slice instead of copying
            random_batch_dataset = dataloader.dataset.select(random_indices)
            random_batch = self.data_collator(random_batch_dataset)
            random_batch = self._prepare_inputs(random_batch)

            policy_output_decoded, ref_output_decoded = self.sample_batch_outputs(self.model, random_batch)

            # Create the wandb table directly without storing in intermediate variables
            try:
                prompts = random_batch_dataset["prompt"]
                game_log_data = [
                    [prompt, pol[len(prompt):], ref[len(prompt):]]
                    for prompt, pol, ref in zip(
                        prompts, policy_output_decoded, ref_output_decoded
                    )
                ]
                # Avoid passing an explicit step; let HF WandbCallback set it
                # Use commit=False so this attaches to the same step as metrics
                wandb.log({
                ...
                print(f"Skipping W&B table logging due to error: {e}")
    ...
    def log(self, logs: Dict[str, float], start_time: Optional[float] = None) -> None:
        ...
        Args:
            ...
            start_time (`Optional[float]`):
                Start timestamp forwarded by `Trainer` for throughput metrics.
        """
        ...
        # Also log the norm of the steering vector, if present
        try:
            with torch.no_grad():
                vec = self.model.model.layers[self.layer].vec  # BlockWrapper/SparseBlockWrapper parameter
                vec_norm = torch.norm(vec).item()
                prefix = "" if train_eval == "train" else "eval"
                logs[f"{prefix}/steering_vec/norm"] = vec_norm
        except Exception:
