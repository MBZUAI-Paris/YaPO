...
class SaveVectorCallback(TrainerCallback):
    """Saves the steering vector to disk at the end of each epoch (not each step)."""

    def __init__(self, layer: int, vec_dir: str, accelerator: Optional[Accelerator] = None, save_every_epochs: int = 1):
    ...
    def on_epoch_end(self, args: TrainingArguments, state: TrainerState, control: TrainerControl, **kwargs):
        ...
        # Determine current epoch number
        ...
        try:
            ...
            return
        if epoch_idx <= 0 or (epoch_idx % self.save_every_epochs != 0):
            return

        model: PreTrainedModel = kwargs.get("model")
        if model is None:
            return
        ...
        try:
            ...
            if not hasattr(layer_module, "vec"):
                return
            vec = layer_module.vec.detach().cpu().clone()
        except Exception:
            return

        try:
            os.makedirs(self.vec_dir, exist_ok=True)
            torch.save(vec, f"{self.vec_dir}/vec_ep{epoch_idx}_layer{self.layer}.pt")
        except Exception:
            # Best-effort; don't crash training if saving fails
            pass
